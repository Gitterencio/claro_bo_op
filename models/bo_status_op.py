...
class BOStatusOp(models.Model):
    ...
    repetir = fields.Boolean(string='Repetir',default=False)
...
class BOStatusOpRec(models.Model):
    ...
    repetir = fields.Boolean(string='Repetir',related='bo_status_op.repetir', readonly=True)
    ...
    def _onchange_bo_status_op_domain(self):
        ...
        if self.oportunidad:
            ...
            if self.oportunidad.status_op_rec_ids:
                ...
                used_status_ids = self.oportunidad.status_op_rec_ids.filtered(lambda r: not r.repetir).mapped('bo_status_op').ids
